...
M.qtype_easyoname = {
    insert_easyoname_applet: function(Y, toreplaceid, appletid, name, topnode, appleturl, feedback, readonly, implicithydrogen, moodleurl, marvinpath) {
        var javaparams = ['mol', Y.one(topnode + ' input.mol').get('value')];
        var easyonameoptions = new Array();
        if (readonly) {
            easyonameoptions[easyonameoptions.length] = Y.one(topnode + ' input.mol').get('value');
        }
        if (easyonameoptions.length !== 0) {
            javaparams[javaparams.length] = "cxsmiles";
            javaparams[javaparams.length] = easyonameoptions.join(',');
        }
        if (!this.show_java(toreplaceid, appletid, name, appleturl, 600, 500, 'chemaxon.marvin.applet.JMSketchLaunch', javaparams, implicithydrogen, moodleurl, marvinpath)) {
    ...
    show_error: function(Y, topnode) {
        var errormessage = '<span class ="javawarning">' + M.util.get_string('enablejava', 'qtype_easyoname') + '</span>';
    ...
    show_java: function(id, appletid, name, java, width, height, appletclass, javavars, implicithydrogen, moodleurl, marvinpath) {
        ...
        param.value = moodleurl + '/question/type/easyoname/customization_mech.xml';
        newApplet.appendChild(param);
        var param = document.createElement('param');
...
M.qtype_easyoname.init_getanswerstring = function(Y, moodle_version) {
    ...
    Y.all(".id_insert").each(function(node) {
        node.on("click", function() {
            ...
            var s = document.MSketch.getMol('cxsmiles:u');
